...
def create_slot(vaccine,dose_choice,age_group,date):  # Creates a instance of a Slot.
    new_slot = Slot()
...
def getSlots(request): # Responses Slot information for the current day.
    ...
    try:
        ...
        if len(slots) == 0: # If no slots are created so far, it creates 8 slots for the current day.
            data.append(create_slot("covishield",'dose1','18to45',date.today()))
            data.append(create_slot("covishield",'dose1','45plus',date.today()))
            data.append(create_slot("covishield",'dose2','18to45',date.today()))
            data.append(create_slot("covishield",'dose2','45plus',date.today()))
            data.append(create_slot("covaxin",'dose1','18to45',date.today()))
            data.append(create_slot("covaxin",'dose1','45plus',date.today()))
            data.append(create_slot("covaxin",'dose2','18to45',date.today()))
            data.append(create_slot("covaxin",'dose2','45plus',date.today()))
        else:
...
def addAndUpdateSlots(request): #Adds a Slot if not present or Update it for the current day.
    ...
    try:
        ...
        if len(slot):
            slot[0].availability = slot[0].availability + data[AVAILABILITY]
            slot[0].save()
        else:
            new_slot = Slot()
            new_slot.vaccine = vaccine
        ...
        for i in range(len(ALL_SLOTS)):
            checkSlot = Slot.objects.filter(vaccine=ALL_SLOTS[i][0],dose_choice=ALL_SLOTS[i][1],age_group=ALL_SLOTS[i][2],date=data[DATE])
            if not len(checkSlot):
                create_slot(ALL_SLOTS[i][0],ALL_SLOTS[i][1],ALL_SLOTS[i][2],data[DATE])
...
def getTokens(request):  # Responses all the tokens generated so far.
    ...
    try:
        ...
        logger.warning("Add Slots: " + str(sys.exc_info()))
        return Response({'action': "Add Slots", 'meyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ1c2VybmFtZSI6InRlc3QiLCJleHAiOjE2MzYyMzQ2NTMsImlhdCI6MTYzNjIzMTA1M30.OE67xhYOQbCQ1qftnwqChyj9srU-qzhQIkbDpADPJEgessage': "Error Occurred {0}".format(
            str(sys.exc_info()[1]))}, 
                        status=status.HTTP_400_BAD_REQUEST)